...
FIXED VERSION - Uses min_periods=1 for rolling statistics
This matches the dashboard implementation!
"""
...
print("Running in CPU-optimized mode - FIXED VERSION")
print("="*70)
...
CONFIG = {
    ...
    'output_dir': r'C:\Users\lenovo\Desktop\projetsmartbuilding',
...
def add_engineered_features(df, window_sizes=[10, 50, 100]):
    ...
    FIXED VERSION - Uses min_periods=1 instead of bfill
    This ensures compatibility with dashboard implementation!
    """
    print("\n" + "="*70)
    print("Engineering Features (FIXED VERSION - min_periods=1)")
    print("="*70)
    ...
    # Rolling statistics - FIXED: Using min_periods=1
    # This matches the dashboard implementation exactly!
    for window in window_sizes:
        # Mean - use min_periods=1 (NOT bfill!)
        df_features[f'vibration_roll_mean_{window}'] = df_features['vibration'].rolling(
            window=window, min_periods=1).mean()
        
        # Std - use min_periods=1
        df_features[f'vibration_roll_std_{window}'] = df_features['vibration'].rolling(
            window=window, min_periods=1).std().fillna(0)
        
        # Max - use min_periods=1
        df_features[f'vibration_roll_max_{window}'] = df_features['vibration'].rolling(
            window=window, min_periods=1).max()
        
        # Min - use min_periods=1
        df_features[f'vibration_roll_min_{window}'] = df_features['vibration'].rolling(
    ...
    print("\n✓ Using min_periods=1 for rolling statistics")
    print("✓ This matches dashboard implementation!")
    print("="*70 + "\n")
...
def main():
    ...
    print("\n" + "="*70)
    print("IMPORTANT: This model now matches dashboard implementation!")
    print("Both use min_periods=1 for rolling statistics.")
    print("="*70)
