...
DATA_PATH = os.path.join(SCRIPT_DIR, 'predictive-maintenance-dataset.csv')
...
def add_features(df):
    ...
    for window in [10, 50, 100]:
        df[f'vibration_roll_mean_{window}'] = df['vibration'].rolling(window=window).mean().fillna(method='bfill')
        df[f'vibration_roll_std_{window}'] = df['vibration'].rolling(window=window).std().fillna(0)
        df[f'vibration_roll_max_{window}'] = df['vibration'].rolling(window=window).max().fillna(method='bfill')
        df[f'vibration_roll_min_{window}'] = df['vibration'].rolling(window=window).min().fillna(method='bfill')
